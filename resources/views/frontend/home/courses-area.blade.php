...
<section class="course-area pb-120px">
    ...
    @endphp
                ...
                <div class="card-body">
                    ...
                    <h5 class="card-title">
        <a href="{{url('course/details/'.$course->id.'/'.$course->course_name_slug)}}">{{$course->course_name}}</a></h5>
                    <p class="card-text">
                    <a href="{{route('instructor.details',$course->instructor_id)}}">{{$course['user']['name']}}</a></p>
                ...
                <span class="rating-number">{{round($average,1)}}</span>
                    ...
                    <span class="rating-total pl-1">({{count($reviews)}})</span>
                    </div><!-- end rating-wrap -->
    ...
    <div class="icon-element icon-element-sm shadow-sm cursor-pointer" title="Add to Wishlist" id="{{ $course->id }}" onclick="addToWishList(this.id)"><i class="la la-heart-o"></i></div>
        ...
        @foreach ($categories as $category )    
                    <div class="tab-pane fade" id="business{{$category->id}}" role="tabpanel" aria-labelledby="design-tab">
                        <div class="row">
@php
...
@endphp
       @forelse ($catwiseCourse as $course )
                ...
                <div class="col-lg-4 responsive-column-half">
                    ...
                    <p class="card-text">
                    <a href="teacher-detail.html">{{$course['user']['name']}}</a></p>
                            <div class="rating-wrap d-flex align-items-center py-2">
                                <div class="review-stars">
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                     @if ($course->discount_price == NULL)
                             <p class="card-price text-black font-weight-bold">&{{ $course->discount_price}}</p>
                        @else
                            <p class="card-price text-black font-weight-bold">&{{ $course->selling_price}}<span class="before-price font-weight-medium">&{{ $course->discount_price}}</span></p>
                        @endif
           ...
           @empty
           <h3 class="text-danger">No Course Found</h3>
       @endforelse
                        </div><!-- end row -->
                    </div><!-- end tab-pane -->
                         @endforeach 
        ...
        <div class="card card-item">
            <div class="card-body">
                ...
                <ul class="generic-list-item generic-list-item-bullet generic-list-item--bullet d-flex align-items-center fs-14">
                    <li>{{$item->duration}} totle hours</li>
                    <li>{{$item->label}}</li>
                </ul>
